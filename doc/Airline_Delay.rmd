...
```{r data clean}
cust <- read_csv('output/combind_data.csv')
cust$airline <- factor(cust$airline)
cust$airline <- revalue(cust$airline, c('ALASKA AIRLINES'='AS',
  ...
  as.character()
cust <- cust %>% 
  mutate(`Carrier`=airline,
               `Missing Luggage`=missing.luggage.per.1000.passangers,
               `Customer Complaint`= `customer complaint`,
               `Involuntary Denied Boarding`=`Involuntary denied boarding.per.1000.passangers`,
               `All Denied Boarding`=`All denied boarding .per.1000.passangers`) %>%
  select(`Carrier`:`All Denied Boarding`)
```

```{r airl summ}
airl_summ <- aot_2018 %>%
  mutate(Carrier=OP_UNIQUE_CARRIER) %>%
  select(Carrier, ARR_DELAY, DEP_DELAY, ORIGIN_CITY_NAME, DEST_CITY_NAME) %>%
  filter(Carrier %in% cust$Carrier) %>%
  left_join(cust, by='Carrier')
saveRDS(airl_summ, 'output/Airline_summary.RDS')
```

```{r head map}
#aot_2018 <- readRDS('output/airline_on_time_2018.RDS')
armp <- aot_2018 %>% 
  mutate(city1 = ORIGIN_CITY_NAME, city2 = DEST_CITY_NAME) %>%
  select(city1, city2, ORIGIN, DEST)
apt_sub <- apt %>% 
    filter(V4=='United States') %>%
    mutate(lati_city1=V7, long_city1=V8, ORIGIN=as.character(V5)) %>%
    select(lati_city1, long_city1, ORIGIN)
armp <- armp %>%
  left_join(apt_sub, by='ORIGIN')
apt_sub <- apt_sub %>% 
  mutate(lati_city2=lati_city1, long_city2=long_city1, DEST=ORIGIN) %>%
  select(DEST, lati_city2, long_city2)
armp <- armp %>%
  left_join(apt_sub, by='DEST') %>%
  select(-ORIGIN, -DEST) %>%
  distinct(city1, city2, .keep_all = TRUE)
saveRDS(armp, 'output/Airline_map.RDS')
```
